...
const DiaryPage = () => {
   ...
   const loadMedias = async () => {
      ...
      try {
         const response = await axios.get(
            BASE_API_URL + `/diary/media/${diary.id}?sort=-1&limit=13&offset=0`
         );
   ...
   const loadNextMedias = async () => {
      ...
      try {
         const response = await axios.get(
            BASE_API_URL +
               `/diary/media/${diary.id}?sort=-1&limit=9&offset=${offset}`
         );
   ...
   const upload = async (files) => {
      ...
      if (files.length > 0) {
         try {
            ...
            await axiosMultipart.post(
               BASE_API_URL + "/diary/media/upload",
               formData
            );
            loadMedias();
         } catch (error) {
   ...
   const getMembers = async () => {
      ...
      try {
         const response = await axios.get(
            BASE_API_URL + `/diary/${diary.id}/members`
         );
   ...
   return (
      <div className="flex flex-col">
         ...
         <Buttons
            ...
            activityClick={() => {
               navigate("/activities", { state: diary.id });
            }}
            isMobile={isMobile}
            setIsMenuVisible={(value) => setIsMenuVisible(value)}
         />
         ...
         {settings && (
            <Settings
               close={closeSettings}
               diary={diary}
               update={(name, color, cover, header) =>
                  settingsUpdate(name, color, cover, header)
               }
               isMobile={isMobile}
            />
         )}

         <LoadingModel open={isUpload} message="uploading" />

         <LoadingModel open={isMemberAdding} message="adding" />

         <LoadingModel open={isUpdating} message="updating" />

         {isMenuVisible && (
            <div
               className="slide-out-div"
               style={{ marginTop: "10rem", boxShadow: "0px 0px 10px inset" }}
            >
               <MdCancel
                  style={{
                     color: "black",
               ...
               >
                  <Button variant="contained" onClick={uploadImageClick} style={{ width:"50%", marginLeft:"2rem", marginTop:"1rem" }}>
                     Upload Media
                  </Button>
                  ...
                  <Button
                     ...
                     style={{ width:"50%", marginLeft:"2rem", marginTop:"1rem" }}
                  >
                     Add Member
                  </Button>
