...
const Media = () => {
    ...
    useEffect(() => {
        const getCreator = async () => {
            ...
            try {
                const response = await axios.get(BASE_API_URL+"/user/"+media.owner);
    ...
    useEffect(() => {
        ...
        var sub;
        stompClient.connect({}, () => {
            sub = stompClient.subscribe('/topic/comment/'+media.id, (paylod) => {
                const data = [];
                const cur = JSON.parse(paylod.body);
                const dateTime = getDateAndTime(cur.created);
                cur['date'] = dateTime[0];
                cur['time'] = dateTime[1];
                cur['isShow'] = (messageRef.current.length === 0 || dateTime[0] !== messageRef.current[messageRef.current.length-1].date);
